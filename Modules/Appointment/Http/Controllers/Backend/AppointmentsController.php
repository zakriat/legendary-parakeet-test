...
class AppointmentsController extends Controller
{
    ...
    public function consultationTariffs(Request $request)
    {
        ...
        $doctorUserId = Doctor::whereKey($data['doctor_id'])->value('doctor_id');
        $at = !empty($data['appointment_date']) && !empty($data['appointment_time'])
            ? Carbon::parse($data['appointment_date'] . ' ' . $data['appointment_time'])
            : null;

        $tariffs = ConsultationTariff::query()
            ->where('clinic_service_id', $data['service_id'])
            ->where('status', 1)
            ->where(fn ($q) => $q->whereNull('clinic_id')->orWhere('clinic_id', $data['clinic_id']))
            ->where(fn ($q) => $q->whereNull('doctor_id')->orWhere('doctor_id', $doctorUserId))
            ->when($at, fn ($q) => $q->where(fn ($q) => $q->whereNull('starts_at')->orWhere('starts_at', '<=', $at))
                ->where(fn ($q) => $q->whereNull('ends_at')->orWhere('ends_at', '>=', $at)))
            ->orderByRaw('CASE WHEN doctor_id IS NULL THEN 1 ELSE 0 END')
            ->orderByRaw('CASE WHEN clinic_id IS NULL THEN 1 ELSE 0 END')
            ->orderByDesc('priority')
            ->get()
            ->map(function ($tariff) {
                $price = round((float) $tariff->price, 2);
                $deposit = $tariff->deposit_type === 'fixed'
                    ? min((float) $tariff->deposit_value, $price)
                    : ($tariff->deposit_type === 'percentage'
                        ? min(round($price * ((float) $tariff->deposit_value / 100), 2), $price)
                        : 0);

                return [
                    'id' => $tariff->id,
                    'label' => sprintf('%s — %s (%s, deposit %s)', $tariff->name, number_format($price, 2), ucfirst($tariff->consultation_mode), number_format($deposit, 2)),
                    'price' => $price,
                    'deposit_amount' => $deposit,
                    'remaining_amount' => round($price - $deposit, 2),
                ];
            });
    ...
    public function store(Request $request)
{
    // Legacy admin/API callers send only user_id; the current off-canvas sends
    // both values. Normalising here keeps both request contracts valid.
    if (!$request->filled('patient_id') && $request->filled('user_id')) {
        $request->merge(['patient_id' => $request->input('user_id')]);
    }

    $request->validate([
        'patient_id' => ['required', 'exists:users,id'],
        'user_id' => ['required', 'same:patient_id', 'exists:users,id'],
        'clinic_id' => ['required', 'integer'],
        'doctor_id' => ['required', 'integer'],
        'service_id' => ['required', 'integer'],
    ...
    $booking = app(AppointmentBookingService::class)->pricing($formData);
    $serviceData = $booking['service_data'];
    $formData = array_merge($formData, $booking['attributes']);
...
// dd($formData);
    ...
    app(AppointmentBookingService::class)->storeClinicalHistory(
        $appointment,
        $request->only(['conditions', 'medications', 'allergies', 'social_history', 'family_history', 'observations'])
    );
