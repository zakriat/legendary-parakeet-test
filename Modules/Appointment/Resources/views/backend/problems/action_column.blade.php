...
@hasPermission('edit_encounter') 
    ...
    <a href="{{route("backend.problems.destroy", $data->id)}}" id="delete-{{$module_name}}-{{$data->id}}" class="btn text-danger p-0 fs-5" data-type="ajax" data-method="DELETE" data-token="{{csrf_token()}}" data-bs-toggle="tooltip" title="{{__('messages.delete')}}" data-confirm="{{ __('messages.are_you_sure?', ['form' => ucwords(str_replace('_', ' ', $data->name)) ?? __('Unknown'), 'module' => __('appointment.problems')])}}"><i class="ph ph-trash"></i></a>
    @endhasPermission
</div>
