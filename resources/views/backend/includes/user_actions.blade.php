<div class="text-end">
    ...
    <a href="{{route('backend.users.edit', $data)}}" class="btn btn-primary btn-icon btn-sm" data-bs-toggle="tooltip" data-bs-title="{{__('labels.backend.edit')}}"><i class="ph ph-pencil-simple-line"></i></a>
    <a href="{{route('backend.users.changePassword', $data)}}" class="btn btn-info btn-icon btn-sm" data-bs-toggle="tooltip" data-bs-title="{{__('labels.backend.changePassword')}}"><i class="ph ph-key"></i></a>
    ...
    @if ($data->id != 1)
    <a href="{{route('backend.users.destroy', $data)}}" class="btn btn-danger btn-icon btn-sm" data-method="DELETE" data-token="{{csrf_token()}}" data-bs-toggle="tooltip" data-bs-title="{{__('labels.backend.delete')}}" data-confirm="{{ __('messages.are_you_sure?') }}"> <i class="ph ph-trash"></i></a>
    @endif
